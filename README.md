...
- **Screen Reader Navigation:** NVDA, JAWS, System Access and SAPI narration on Windows, and VoiceOver narration on macOS for all menus and settings.
...
Screen reader output goes through [Tolk](https://github.com/dkager/tolk) on Windows (NVDA, JAWS, System Access and SAPI) and native VoiceOver integration on macOS.

## For players

Grab `PolygonBitAccessibility.zip` from the release, extract it into the game folder next to
`PolygonBitBattleRoyale.exe` (Windows) or `PolygonBitBattleRoyale.app` (macOS), and launch from Steam (Windows) or run `./run_bepinex.sh` from the game folder (macOS). BepInEx is bundled — there is nothing else to
install.
...
2. copies the mod into your game's `BepInEx\plugins\AccessibilityMod` (and Tolk natives on Windows)
...
`GAME_DIR` at the top of `deploy.bat` or `deploy.sh` is the only thing you may need to change. The BepInEx core in
...
| `lib/Tolk/x64/` | Tolk and its screen reader clients (Windows) |
...
- [Tolk](https://github.com/dkager/tolk) by Davy Kager — screen reader abstraction (Windows)
- VoiceOver Objective-C bridge — VoiceOver narration (macOS)
